...
class _TaskDetailPageState extends State<TaskDetailPage> {
  ...
  Future<void> _updateStatus(String status) async {
    if (_task == null) return;
    final updated = _task!.copyWith(status: status, progress: status == 'completed' ? 100 : _task!.progress);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              _badge(_priorityLabel(task.priority), _priorityColor(task.priority)),
            ...
            if (task.description != null) ...[
              ...
              Text(task.description!, style: TextStyle(fontSize: 16, color: Colors.grey[600])),
            ],
              ...
              child: Padding(
                ...
                child: Column(children: [
                  if (task.startDate != null) ...[
                    _infoRow(Icons.calendar_today, '开始日期', '${task.startDate!.year}-${task.startDate!.month.toString().padLeft(2, '0')}-${task.startDate!.day.toString().padLeft(2, '0')}'),
                    const Divider(),
                  ],
                  if (task.endDate != null) ...[
                    _infoRow(Icons.event, '截止日期', '${task.endDate!.year}-${task.endDate!.month.toString().padLeft(2, '0')}-${task.endDate!.day.toString().padLeft(2, '0')}'),
                    const Divider(),
                  ],
            ...
            const SizedBox(height: 8),
            Text('${task.progress}% 完成', style: TextStyle(color: Colors.grey[600])),
            const SizedBox(height: 24),
            Row(children: [
              Expanded(
  ...
  String _priorityLabel(String p) => switch (p) { 'P0' => '紧急', 'P1' => '重要', 'P2' => '普通', _ => '低' };
  String _statusLabel(String s) => switch (s) { 'pending' => '待办', 'in_progress' => '进行中', 'completed' => '已完成', 'failed' => '失败', _ => s };

  Color _priorityColor(String p) => switch (p) { 'P0' => Colors.red, 'P1' => Colors.orange, 'P2' => Colors.green, _ => Colors.blue };
  Color _statusColor(String s) => switch (s) { 'pending' => Colors.grey, 'in_progress' => Colors.blue, 'completed' => Colors.green, 'failed' => Colors.red, _ => Colors.grey };
}
