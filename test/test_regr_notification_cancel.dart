// 回归测试: 提醒取消链路降量（1834bce Windows Toast + 本次 P1-F cancelRepeats 折叠）
...
// 对应提交: bf3e1cb 工作树（P1-F 修改 notification_service_io.dart:887-900）
// 本测试用 dart:io 读源文件做结构不变量断言（NotificationService 依赖平台通道无法无头实例化）
// Layer: REGRESSION
// TestedSource: NotificationService.cancelReminderForSchedule@bf3e1cb072553db828e25b7fd3b67fdc2e61b50d
...
//     若断言无 cancelRepeats 参数 → 本测试 FAIL
// Q3: 新代码在这条件下会怎样？
// A3: cancelRepeats:false → 取消 base+offset1 后 `if (!cancelRepeats) return;` 跳过循环（仅 2 次）；
//     默认 true 保持全量取消；本测试断言参数 + 守卫 + 循环在守卫后 → PASS
//
// === 入参/出参标准 ===
// 入参: notification_service_io.dart + notification_service_web.dart 源文件
// 出参: 结构断言 — cancelRepeats 参数存在 / `if (!cancelRepeats) return;` 在 base+offset1 取消后 /
//       repeat 循环在守卫后 / web 签名同步
...
void main() {
  ...
  group('P1-F cancelRepeats 折叠（io 实现）', () {
    ...
    List<String> activeLines(String marker) => io
        .split('\n')
    ...
    test('base+offset1 取消先于守卫，repeat 循环在守卫后', () {
      // 顺序：cancelNotification(base) → cancelNotification(offset1) → if (!cancelRepeats) return
      ...
      final offset1Idx = body.indexOf('notificationIdForSchedule(scheduleId, offset: 1)');
      ...
      expect(guardLines, isNotEmpty, reason: 'cancelRepeats 短路守卫缺失（活动代码）');
    ...
    test('批量重调度调用点传 cancelRepeats: false', () {
      expect(activeLines('cancelReminderForSchedule(task.id, cancelRepeats: false)'),
          isNotEmpty, reason: '批量重调度路径未降量（活动代码）');
    });
  });

  group('web 空实现签名同步', () {
    test('web cancelReminderForSchedule 含 cancelRepeats 参数', () {
      expect(web.contains('bool cancelRepeats = true,'), isTrue,
          reason: 'web 签名未同步（5749d13 web stub 兼容性破坏）');
    });
  });
}
// === 破坏性验证 ===
// 注释 notification_service_io.dart:893 的 `if (!cancelRepeats) return;` 后运行:
//   flutter test test_regr_notification_cancel.dart
// 预期: FAIL (AssertionError: guard 须在 base+offset1 取消之后)
// 恢复后: PASS
