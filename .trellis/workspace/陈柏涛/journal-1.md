...
将 calendar_date_picker.dart 分钟下拉框替换为 TextField + ▲▼ 步进按钮，支持 0–59 任意值手动输入，失焦自动校验，步进边界循环。
...
## Session 19: 折扣码表单UX优化：下拉选类型+自动推荐码

**Date**: 2026-06-09
**Task**: 折扣码表单UX优化：下拉选类型+自动推荐码
**Branch**: `master`

### Summary

将后台管理折扣码表单的'适用会员类型'字段从手填UUID文本框改为动态下拉选择器；新增折扣码自动推荐（选类型+比例后自动生成如PRO80），用户手动改后不再覆盖。代码在taskora-website仓库(781447f)。
...
## Session 20: 桌面端提醒持久化：Windows Toast不自动消失

**Date**: 2026-06-09
**Task**: 桌面端提醒持久化：Windows Toast不自动消失
**Branch**: `master`

### Summary

Windows提醒改为系统Toast（reminder场景不自动消失），支持稍后提醒下拉/标记完成/查看详情/知道了按钮。窗口X改为隐藏到托盘保持Timer运行。移动端通知路径完全不受影响。
